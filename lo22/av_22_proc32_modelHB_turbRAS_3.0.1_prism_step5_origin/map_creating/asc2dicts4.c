...
/*asc2dicts [map ascii] [divided times] [snow depth] [region map ascii]*/
...
//	fclose(f);
	...
	float snow_depth_simple = atof(argv[3]);
	printf("snow depth = %f\n", snow_depth_simple);
...
exit_without_massives:
	printf("asc2dicts [map in ascii format] [divided times] [snow depth] [region map ascii]\n");
	return 1;
}
